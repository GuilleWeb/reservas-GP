...
<div class="max-w-7xl mx-auto">
  ...
  <div class="lg:col-span-4">
    ...
    <div class="text-xl font-extrabold text-gray-900 mb-6">Agendar Cita (Admin)</div>
...
<script>
  ...
  function loadOptions() {
    $.get('<?= app_url('api/admin/citas.php') ?>', { action: 'get_options' }, function (res) {
      if (res.success) {
  ...
  function loadData(page = 1) {
    ...
    $.get('<?= app_url('api/admin/citas.php') ?>', { action: 'list', page: currentPage, per: per, search: search, estado: estado }, function (res) {
      ...
      if (res.success && res.data.length > 0) {
        ...
        tbody.html('<tr><td colspan="4" class="py-10 text-center text-gray-500">No hay citas registradas en este estado.</td></tr>');
  ...
  function editItem(id) {
    $.get('<?= app_url('api/admin/citas.php') ?>', { action: 'get', id: id }, function (res) {
  ...
  function deleteItem(id) {
    showCustomConfirm("¿Deseas de cancelar/eliminar esta cita?", function () {
      $.post('<?= app_url('api/admin/citas.php') ?>', { action: 'delete', id: id }, function (res) {
        if (res.success) {
          loadData(currentPage);
          showCustomAlert('Cita cancelada/eliminada.', 5000, 'info');
        } else {
          showCustomAlert(res.message || 'Error al procesar', 5000, 'error');
        }
      }, 'json');
    });
  }
  ...
  $(function () {
    ...
    $('#formCita').on('submit', function (e) {
      e.preventDefault();
      $.post('<?= app_url('api/admin/citas.php') ?>?action=save', $(this).serialize(), function (res) {
        if (res.success) {
          resetForm();
          loadData(currentPage);
          showCustomAlert('Cita agendada/actualizada correctamente.', 5000, 'success');
        } else {
          showCustomAlert(res.message || 'Error al agendar cita', 5000, 'error');
        }
      }, 'json');
    });
