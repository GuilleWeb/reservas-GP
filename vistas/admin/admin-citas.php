...
<div class="max-w-7xl mx-auto">
  <div class="bg-white dark:bg-slate-900/60 dark:border-slate-800 shadow rounded-2xl p-6 border min-h-[500px]">
    ...
    <div class="mb-4 grid grid-cols-1 md:grid-cols-6 gap-3">
      <input id="txtSearch" type="text" placeholder="Buscar por cliente, email o empleado..." class="border rounded-lg p-2 md:col-span-3">
...
<script>
  const API_URL = '<?= app_url('api/admin/citas.php') ?>?id_e=<?= urlencode((string) request_id_e()) ?>';
  let currentPage = 1;
        ...
        res.data.forEach(item=>{
          ...
          tbody.append(`
            <tr class="hover:bg-teal-50/30 transition-colors">
              <td class="py-3 px-4 font-semibold text-gray-800 text-sm">${dateStr}<br><span class="text-xs text-gray-500 font-normal">Sede: ${item.sucursal_nombre || 'N/A'}</span></td>
              <td class="py-3 px-4"><div class="text-sm font-semibold">${item.cliente_nombre || '-'}</div><div class="text-xs text-gray-500">${item.servicio_nombre || '-'} · ${item.empleado_nombre || '-'}</div></td>
              <td class="py-3 px-4">${getEstadoBadge(item.estado)}</td>
              <td class="py-3 px-4 text-right"><button onclick="viewDetails(${item.id})" class="text-teal-600 hover:text-teal-800 bg-teal-50 px-2.5 py-1.5 rounded-lg border border-teal-200"><i data-lucide="eye"></i></button></td>
            </tr>
          `);
        });
  ...
  function loadCalendarRange(){
    const r=getRange();
    $.get(API_URL,{action:'calendar', from:toDateKey(r.from), to:toDateKey(r.to)}, function(res){
  ...
  function renderCalendar(){
    ...
    if(calendarMode==='month'){
      ...
      const startOffset=(firstDay.getDay()+6)%7; for(let i=0;i<startOffset;i++) days.push(null); for(let d=1;d<=lastDay.getDate();d++) days.push(new Date(cursorDate.getFullYear(),cursorDate.getMonth(),d));
      label=monthFmt.format(cursorDate); $('#weekdayHead').removeClass('hidden'); grid.removeClass('grid-cols-1').addClass('grid-cols-7');
    }else if(calendarMode==='week'){
      const s=startOfWeek(cursorDate); for(let i=0;i<7;i++) days.push(new Date(s.getFullYear(),s.getMonth(),s.getDate()+i)); const e=endOfWeek(cursorDate);
      label=`${s.toLocaleDateString()} - ${e.toLocaleDateString()}`; $('#weekdayHead').removeClass('hidden'); grid.removeClass('grid-cols-1').addClass('grid-cols-7');
    }else{
      const d=new Date(cursorDate.getFullYear(),cursorDate.getMonth(),cursorDate.getDate()); days=[d]; label=dayFmt.format(d); $('#weekdayHead').addClass('hidden'); grid.removeClass('grid-cols-7').addClass('grid-cols-1');
    }
    $('#rangeLabel').text(label);
    days.forEach(day=>{
      if(!day){ grid.append('<div class="h-28 border-r border-b bg-gray-50/50"></div>'); return; }
      const key=toDateKey(day), count=(grouped[key]||[]).length;
      ...
      const badge=count>0?`<span class="inline-flex items-center justify-center text-[11px] px-2 py-0.5 rounded-full bg-teal-100 text-teal-700 font-semibold">${count}</span>`:'<span class="text-[11px] text-gray-300">Sin citas</span>';
      const hClass=calendarMode==='day'?'h-32':'h-20';
      grid.append(`<button type="button" class="${hClass} ${today} ${selected} p-2 text-left hover:bg-teal-50/60 transition day-cell rounded-xl border shadow-sm bg-white" data-date="${key}"><div class="flex items-center justify-between"><span class="text-sm font-semibold text-gray-800">${day.getDate()}</span>${badge}</div><div class="mt-2 space-y-1">${(grouped[key]||[]).slice(0, calendarMode==='day'?5:2).map(it=>{const dt=parseSqlDate(it.inicio); const hh=dt.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}); return `<div class="text-[11px] truncate text-gray-600">${hh} · ${it.cliente_nombre || 'Cliente'}</div>`;}).join('')}</div></button>`);
    });
  }
  ...
  function renderDayAgenda(dateKey){
    ...
    const items=calendarItems.filter(c=>toDateKey(parseSqlDate(c.inicio))===dateKey).sort((a,b)=>parseSqlDate(a.inicio)-parseSqlDate(b.inicio));
    const d=parseSqlDate(dateKey+' 00:00:00'); $('#dayAgendaTitle').text(`Citas del ${dayFmt.format(d)}`);
  ...
  function openModal(item){
    ...
    $('#btnSaveModal').addClass('hidden'); $('#btnEditModal').removeClass('hidden');
  ...
  function saveModal(){
    ...
    $.post(API_URL,{action:'update_status',id:modalItemId,estado:$('#mEstado').val(),notas:$('#mNotas').val()},function(res){
      if(res&&res.success){ showCustomAlert('Cita actualizada.',3000,'success'); closeModal(); loadTable(currentPage); if(currentView==='calendar') loadCalendarRange(); }
      else showCustomAlert((res&&res.message)||'No se pudo actualizar.',5000,'error');
    },'json');
  }
