...
<script>
  const API_RESENAS = <?= json_encode(app_url('api/sucursal/resenas.php')) ?>;
  let currentPage = 1;
  ...
  function loadData(page = 1) {
    currentPage = page;
    $.get(API_RESENAS, { action: 'list', page: currentPage, per: 15 }, function (res) {
      const tbody = $('#tableBody').empty();
      if (res.success && res.data.length > 0) {
        res.data.forEach(item => {
          let stars = '';
          for (let i = 1; i <= 5; i++) {
            stars += i <= parseInt(item.rating) ? '<i class="fas fa-star text-yellow-500"></i>' : '<i class="far fa-star text-yellow-300"></i>';
          }

          let visible = parseInt(item.visible_en_home) ? `<span class="bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded text-xs">Home Page</span>` : '';
  ...
  function editItem(id) {
    $.get(API_RESENAS, { action: 'get', id: id }, function (res) {
  ...
  function deleteItem(id) {
    if (!confirm("¿Deseas desactivar esta reseña?")) return;
    $.post(API_RESENAS, { action: 'delete', id: id }, function (res) {
  ...
  $(function () {
    $('#formResena').on('submit', function (e) {
      ...
      $.post(API_RESENAS + '?action=save', data, function (res) {
        if (res.success) {
          resetForm();
