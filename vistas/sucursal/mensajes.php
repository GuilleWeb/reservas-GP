    ...
    <div class="lg:col-span-4">
      <div class="bg-white rounded-2xl shadow p-5 border">
        <div class="flex justify-between items-start">
          <div>
            <div class="text-sm text-gray-500">Comunicación interna</div>
        ...
        <form id="sendForm" class="mt-4 space-y-3">
          <div>
            ...
            <select id="modo" name="modo" class="border rounded-lg p-2 w-full">
              ...
              <option value="one">Empleado específico</option>
            </select>
          </div>
    ...
    <div class="lg:col-span-8">
      <div class="bg-white rounded-2xl shadow border p-5">
        <div>
          ...
          <div class="text-sm text-gray-500">Recibidos y enviados del gerente.</div>
        </div>
          ...
          <table class="w-full text-left border-collapse min-w-max">
            <thead class="bg-white border-b sticky top-0 z-10 shadow-sm">
              <tr>
                ...
                <th class="px-4 py-3 text-xs font-semibold text-gray-600 uppercase">Remitente / Destino</th>
...
<script>
  ...
  $(function () {
    const API = <?= json_encode(app_url('api/sucursal/mensajes.php') . '?id_e=' . urlencode((string) request_id_e())) ?>;
    let page = 1, per = 10, search = '', estado = '', folder = 'inbox';
    let currentId = 0;
    let timer = null;
    ...
    function loadTargets() {
      $.get(API, { action: 'list_targets' }, function (res) {
        const s = $('#target_user_id').empty();
        s.append('<option value="0">Selecciona empleado</option>');
        (res.data || []).forEach(u => s.append(`<option value="${u.id}">${u.nombre || ''} (${u.email || ''})</option>`));
      }, 'json');
    }
    function loadInbox() {
      $.get(API, { action: 'list', page, per, search, estado, folder }, function (res) {
        if (!res || !res.success) return;
        $('#totalReg').text(`Total: ${res.total || 0}`);
        const tb = $('#msgTable').empty();
        (res.data || []).forEach(m => {
          const contacto = folder === 'sent' ? ('Yo → ' + (m.remitente || 'Destino')) : (m.remitente || '');
          tb.append(`<tr class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-right"><button class="h-8 w-8 grid place-items-center rounded-lg border viewBtn" data-id="${m.id}"><i data-lucide="eye"></i></button></td>
          </tr>`);
        });
    ...
    $('#fFolder').on('change', function () { folder = $(this).val() || 'inbox'; page = 1; loadInbox(); });
    ...
    $('#msgTable').on('click', '.viewBtn', function () {
      ...
      $.get(API, { action: 'get', id, folder }, function (res) {
        ...
        $('#mRemitente').text(folder === 'sent' ? ('Yo → ' + (d.remitente || 'Destino')) : (d.remitente || ''));
        $('#mFecha').text(d.created_at || '');
        $('#mCuerpo').html(d.cuerpo || '');
        $('#msgModal').removeClass('hidden');
        if (folder !== 'sent' && String(d.estado || '') === 'nuevo') {
          $.post(API, { action: 'set_estado', id: currentId, estado: 'leido', folder }, function () { loadInbox(); }, 'json');
        }
      }, 'json');
    });
    ...
    $('#btnDelete').on('click', function () {
      ...
      $.post(API, { action: 'set_estado', id: currentId, estado: 'eliminado', folder }, function () { $('#msgModal').addClass('hidden'); loadInbox(); }, 'json');
    });
    $('#btnResend').on('click', function () {
      const titulo = ($('#mTitulo').text() || '').trim();
      const cuerpo = ($('#mCuerpo').html() || '').trim();
      if (titulo) $('#sendForm [name="titulo"]').val(titulo);
      if (cuerpo) quill.root.innerHTML = cuerpo;
