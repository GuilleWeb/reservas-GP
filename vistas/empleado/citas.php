  ...
  <div class="bg-white shadow rounded-2xl p-6 border min-h-[500px]">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500">Vista tabla y calendario (mes, semana y día) para tus citas.</p>
      </div>
    ...
    <div id="tableView" class="hidden">
      <div id="tableRangeControls" class="mb-4 flex flex-wrap items-center gap-2">
...
<div id="citaModal" class="fixed inset-0 bg-black/40 z-50 hidden items-center justify-center p-4">
  <div class="bg-white rounded-2xl shadow-xl border w-full max-w-2xl p-5">
    ...
    <div class="mt-4 pt-4 border-t flex items-center justify-between">
      <div class="text-xs text-gray-500 max-w-md">Nota: si la cita ya superó su hora de fin, se marca como completada automáticamente y ya no podrá editarse por empleado.</div>
...
<script>
  const API_URL = '<?= app_url('api/empleado/citas.php') ?>?id_e=<?= urlencode((string) request_id_e()) ?>';
  let currentPage = 1;
  let currentView = 'calendar';
  let calendarMode = 'day';
  let tableRangeMode = 'all';
  ...
  function refreshTableRangeButtons() {
    ...
    pairs.forEach(([sel, mode]) => $(sel).toggleClass('bg-teal-600 text-white border-teal-600', tableRangeMode === mode));
  }
  function getRange() {
    if (calendarMode === 'day') {
      const d = new Date(cursorDate.getFullYear(), cursorDate.getMonth(), cursorDate.getDate());
      return { from: d, to: d };
    }
    if (calendarMode === 'week') {
      return { from: startOfWeek(cursorDate), to: endOfWeek(cursorDate) };
    }
    return {
      from: new Date(cursorDate.getFullYear(), cursorDate.getMonth(), 1),
      to: new Date(cursorDate.getFullYear(), cursorDate.getMonth() + 1, 0),
    };
  }
  ...
  function renderPagination(totalPages, current) {
    ...
    const nextDisabled = current >= totalPages ? 'opacity-50 pointer-events-none' : '';
    ...
    for (let i = 1; i <= totalPages; i++) {
      if (i === 1 || i === totalPages || (i >= current - 1 && i <= current + 1)) {
        const active = i === current ? 'bg-teal-600 text-white border-teal-600' : 'border hover:bg-gray-50';
        pag.append(`<button onclick="loadTable(${i})" class="px-3 py-1 rounded ${active}">${i}</button>`);
      } else if (i === current - 2 || i === current + 2) {
        pag.append('<span class="px-2 text-gray-400">...</span>');
      }
    }
  ...
  function loadTable(page = 1) {
    ...
    $.get(API_URL, { action: 'list', page: currentPage, per: 15, from: range.from, to: range.to }, function (res) {
      const tbody = $('#tableBody').empty();
      byId = {};
      if (res.success && res.data.length > 0) {
        res.data.forEach(item => {
          byId[item.id] = item;
          const d = parseSqlDate(item.inicio);
          ...
          tbody.append(`
            <tr class="hover:bg-teal-50/30 transition-colors">
              ...
              <td class="py-3 px-4">${getEstadoBadge(item.estado)}</td>
              <td class="py-3 px-4 text-right">
                <button onclick="viewDetails(${item.id})" class="text-teal-600 hover:text-teal-800 bg-teal-50 px-2.5 py-1.5 rounded-lg border border-teal-200">
                  <i data-lucide="eye"></i>
                </button>
              </td>
            </tr>
  ...
  function loadCalendarRange() {
    ...
    $.get(API_URL, { action: 'calendar', from: toDateKey(range.from), to: toDateKey(range.to) }, function (res) {
      calendarItems = (res.success && Array.isArray(res.data)) ? res.data : [];
      calendarItems.forEach(it => byId[it.id] = it);
      renderCalendar();
  ...
  function renderCalendar() {
    ...
    calendarItems.forEach(c => {
      const k = toDateKey(parseSqlDate(c.inicio));
      grouped[k] = grouped[k] || [];
      grouped[k].push(c);
    });
    ...
    if (calendarMode === 'month') {
      ...
      for (let d = 1; d <= lastDay.getDate(); d++) days.push(new Date(cursorDate.getFullYear(), cursorDate.getMonth(), d));
      label = monthFmt.format(cursorDate);
      $('#weekdayHead').removeClass('hidden');
      grid.removeClass('grid-cols-1').addClass('grid-cols-7');
      ...
      $('#weekdayHead').addClass('hidden');
      grid.removeClass('grid-cols-7').addClass('grid-cols-1');
    }

    $('#rangeLabel').text(label);
    days.forEach(day => {
      if (!day) {
        grid.append('<div class="h-28 border-r border-b bg-gray-50/50"></div>');
        return;
      }
      const key = toDateKey(day);
      const count = (grouped[key] || []).length;
      const selected = key === selectedDate ? 'ring-2 ring-teal-500 bg-teal-50' : 'bg-white';
      const today = key === todayKey ? 'border-teal-300' : 'border-gray-100';
      const badge = count > 0
        ? `<span class="inline-flex items-center justify-center text-[11px] px-2 py-0.5 rounded-full bg-teal-100 text-teal-700 font-semibold">${count}</span>`
        : '<span class="text-[11px] text-gray-300">Sin citas</span>';
      const hClass = calendarMode === 'day' ? 'h-32' : 'h-20';
      grid.append(`
        <button type="button" class="${hClass} ${today} ${selected} p-2 text-left hover:bg-teal-50/60 transition day-cell rounded-xl border shadow-sm bg-white" data-date="${key}">
          <div class="flex items-center justify-between">
            <span class="text-sm font-semibold text-gray-800">${day.getDate()}</span>
            ${badge}
          </div>
          <div class="mt-2 space-y-1">
            ${(grouped[key] || []).slice(0, calendarMode === 'day' ? 5 : 2).map(it => {
              const dt = parseSqlDate(it.inicio);
              const hh = dt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
              return `<div class="text-[11px] truncate text-gray-600">${hh} · ${it.cliente_nombre || 'Cliente'}</div>`;
            }).join('')}
          </div>
        </button>
      `);
  ...
  function renderDayAgenda(dateKey) {
    ...
    $('.day-cell').removeClass('ring-2 ring-teal-500 bg-teal-50');
    $(`.day-cell[data-date="${dateKey}"]`).addClass('ring-2 ring-teal-500 bg-teal-50');
    const items = calendarItems
      .filter(c => toDateKey(parseSqlDate(c.inicio)) === dateKey)
      .sort((a, b) => parseSqlDate(a.inicio) - parseSqlDate(b.inicio));
    const d = parseSqlDate(dateKey + ' 00:00:00');
    $('#dayAgendaTitle').text(`Citas del ${dayFmt.format(d)}`);
    const box = $('#dayAgenda').empty();
    if (!items.length) {
      box.html('<div class="text-sm text-gray-500">No hay citas para este día.</div>');
      return;
    }
    items.forEach(item => {
      const start = parseSqlDate(item.inicio).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      const end = item.fin ? parseSqlDate(item.fin).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--:--';
      box.append(`
        <button type="button" onclick="viewDetails(${item.id})" class="w-full text-left bg-white rounded-lg border p-3 flex flex-col gap-2 hover:bg-teal-50/40">
          <div class="font-semibold text-gray-800">${start} - ${end}</div>
          <div class="text-sm text-gray-600">${item.cliente_nombre || '-'} · ${item.servicio_nombre || '-'}</div>
          <div class="text-xs text-gray-500">Sede: ${item.sucursal_nombre || 'N/A'}</div>
  ...
  function switchView(mode) {
    ...
    $('#tableView').toggleClass('hidden', !isTable);
    $('#calendarView').toggleClass('hidden', isTable);
    $('#btnViewTable').toggleClass('bg-teal-600 text-white', isTable).toggleClass('text-gray-700', !isTable);
    $('#btnViewCalendar').toggleClass('bg-teal-600 text-white', !isTable).toggleClass('text-gray-700', isTable);
    if (isTable) loadTable(currentPage || 1);
    else loadCalendarRange();
  }

  function setCalendarMode(mode) {
    calendarMode = mode;
    $('#btnModeMonth').toggleClass('bg-teal-600 text-white', mode === 'month').toggleClass('text-gray-700', mode !== 'month');
    $('#btnModeWeek').toggleClass('bg-teal-600 text-white', mode === 'week').toggleClass('text-gray-700', mode !== 'week');
    $('#btnModeDay').toggleClass('bg-teal-600 text-white', mode === 'day').toggleClass('text-gray-700', mode !== 'day');
    loadCalendarRange();
  }
  ...
  function openModal(item) {
    ...
    const fecha = parseSqlDate(item.inicio);
    const fin = item.fin ? parseSqlDate(item.fin) : null;
    const total = parseFloat(item.servicio_precio || 0).toFixed(2);
    const rows = [
      ['Cliente', item.cliente_nombre || '-'],
      ['Servicio', item.servicio_nombre || '-'],
      ['Sucursal', item.sucursal_nombre || '-'],
      ['Dirección sede', item.sucursal_direccion || '-'],
      ['Detalle servicio', item.servicio_descripcion || '-'],
      ['Duración', `${item.servicio_duracion_minutos || '-'} min`],
      ['Total', `$${total}`],
      ['Fecha', fecha.toLocaleDateString()],
      ['Hora', `${fecha.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}${fin ? ' - ' + fin.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}`],
      ['Teléfono', item.cliente_telefono || '-'],
    ];
    rows.forEach(r => info.append(`<div class="bg-gray-50 border rounded-lg p-2"><div class="text-xs text-gray-500">${r[0]}</div><div class="font-medium text-gray-800">${r[1]}</div></div>`));
  ...
  function viewDetails(id) {
    ...
    $.get(API_URL, { action: 'get', id }, function (res) {
  ...
  function closeModal() {
    $('#citaModal').addClass('hidden').removeClass('flex');
  }

  function saveModal() {
    if (modalItemId <= 0) return;
    $.post(API_URL, {
      ...
      if (res && res.success) {
        showCustomAlert('Cita actualizada.', 3000, 'success');
        closeModal();
        loadTable(currentPage);
        if (currentView === 'calendar') loadCalendarRange();
      } else {
  ...
  $(function () {
    ...
    $('#btnPrevRange').on('click', function () {
      if (calendarMode === 'month') cursorDate = new Date(cursorDate.getFullYear(), cursorDate.getMonth() - 1, 1);
      else if (calendarMode === 'week') cursorDate = new Date(cursorDate.getFullYear(), cursorDate.getMonth(), cursorDate.getDate() - 7);
      else cursorDate = new Date(cursorDate.getFullYear(), cursorDate.getMonth(), cursorDate.getDate() - 1);
      loadCalendarRange();
    });
    $('#btnNextRange').on('click', function () {
      if (calendarMode === 'month') cursorDate = new Date(cursorDate.getFullYear(), cursorDate.getMonth() + 1, 1);
      else if (calendarMode === 'week') cursorDate = new Date(cursorDate.getFullYear(), cursorDate.getMonth(), cursorDate.getDate() + 7);
      else cursorDate = new Date(cursorDate.getFullYear(), cursorDate.getMonth(), cursorDate.getDate() + 1);
      loadCalendarRange();
